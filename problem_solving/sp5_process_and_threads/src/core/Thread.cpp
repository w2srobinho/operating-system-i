 ...
 * lista de threads no sistema, inseri-la na fila de prontos invocando o método void Scheduler::insert(Thread* thread),
...
Thread* Thread::thread_create(Process* parent) {
    ...
    // INSERT YOUR CODE HERE
    auto newThread = new Thread(parent);
    getThreadsList()->push_back(newThread);
    Scheduler::insert()
